...
/// file, which is typically located at `$XDG_CONFIG_HOME/niftty/config`.
/// On macOS, this may also be located at
/// `~/Library/Application Support/com.niftty.app/config`.
...
fn runInner(alloc: Allocator, stderr: *std.Io.Writer) !u8 {
    ...
    if (comptime builtin.os.tag == .windows) {
        try stderr.print(
            \\The `niftty ++edit-config` command is not supported on Windows.
