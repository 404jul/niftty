...
const usage =
    \\Usage: ghostty +ssh-files --pid=<pid> list <absolute-remote-directory>
    \\       ghostty +ssh-files --pid=<pid> stat <absolute-remote-path>
    \\       ghostty +ssh-files --pid=<pid> download <absolute-remote-file> <absolute-local-path>
    \\       ghostty +ssh-files --pid=<pid> mkdir <absolute-remote-directory>
    \\       ghostty +ssh-files --pid=<pid> rename <absolute-old-path> <absolute-new-path>
    \\       ghostty +ssh-files --pid=<pid> delete <absolute-remote-path>
    \\
    \\File operations over the SFTP subsystem of an active `ghostty +ssh` session.
