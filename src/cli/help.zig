...
pub fn run(alloc: Allocator) !u8 {
    ...
    try stdout.writeAll(
        ...
        \\`niftty -e top` will run the `top` command inside the terminal.
        \\
        \\On macOS, launching the terminal emulator from the CLI is not
        \\supported and only actions are supported. Use `open -na Niftty.app`
        \\instead, or `open -na Niftty.app --args --foo=bar --baz=quz` to pass
        \\arguments.
        \\
        \\Available actions:
        \\
