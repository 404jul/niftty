...
/// Set Niftty's graphical user interface language to a language other than the
/// system default language. For example:
///
///     language = de
///
/// will force the strings in Niftty's graphical user interface to be in German
/// rather than the system default.
///
/// This will not affect the language used by programs run _within_ Niftty.
/// Those will continue to use the default system language. There are also many
/// non-GUI elements in Niftty that are not translated - this setting will have
/// no effect on those.
///
/// Warning: This setting cannot be reloaded at runtime. To change the language
/// you must fully restart Niftty.
///
/// GTK only.
...
/// etc.
///
/// Notes on emoji specifically: On macOS, Niftty by default will always use
...
/// found, Niftty will use the regular style. This prevents falling back to a
...
/// set `font-family-bold = FooBar` and "FooBar" cannot be found, Niftty will
/// use whatever font is set for `font-family` for the bold style.
///
/// Finally, some styles may be synthesized if they are not supported.
/// For example, if a font does not have an italic style and no alternative
/// italic font is specified, Niftty will synthesize an italic style by
...
/// Control whether Niftty should synthesize a style if the requested style is
/// not available in the specified font-family.
///
/// Niftty can synthesize bold, italic, and bold italic styles if the font
...
///
/// The font feature will apply to all fonts rendered by Niftty. A future
...
///
/// Niftty breaks runs at certain points to improve readability or usability.
/// For example, Niftty by default will break runs under the cursor so that
...
///
/// This configuration only applies to Niftty builds that use FreeType.
...
///     This is the most common setting in GTK apps and therefore also Niftty's
...
/// name, or an absolute path to a custom theme file. Niftty also supports
/// specifying a different theme to use for light and dark mode. Each
/// option is documented below.
///
/// If the theme is an absolute pathname, Niftty will attempt to load that
...
///
/// The second directory is the `themes` subdirectory of the Niftty resources
/// directory. Niftty ships with a multitude of themes that will be installed
/// into this directory. On macOS, this list is in the
/// `Niftty.app/Contents/Resources/ghostty/themes` directory. On Linux, this
/// list is in the `share/ghostty/themes` directory (wherever you installed the
/// Niftty "share" directory.
///
/// To see a list of available themes, run `niftty +list-themes`.
///
/// A theme file is simply another Niftty configuration file. They share
...
/// `config-file`. At the time of writing this, Niftty will not show any
...
/// splits. A future improvement to Niftty will address this.
///
...
/// memory usage (specifically VRAM usage). A future Niftty improvement
...
///
/// If this is not set at all (`null`), then Niftty will respect DEC Mode 12
...
/// additional configuration or Niftty's shell integration features to be
/// enabled.
///
...
///   anything but modifiers or keybinds that are processed by Niftty).
///
...
///
/// On macOS, changing this configuration requires restarting Niftty completely.
...
/// or `true` value will enable background blur. On Wayland, Niftty uses
...
/// Control when Niftty preserves a zoomed split. Under normal circumstances,
...
/// created when Niftty starts, use the `initial-command` configuration.
///
/// Niftty supports the common `-e` flag for executing a command with
/// arguments. For example, `niftty -e fish --with --custom --args`.
/// This flag sets the `initial-command` configuration, see that for more
/// information.
command: ?Command = null,

/// This is the same as "command", but only applies to the first terminal
/// surface created when Niftty starts. Subsequent terminal surfaces will use
...
///
/// If you're using the `niftty` CLI there is also a shortcut to set this
/// with arguments directly: you can use the `-e` flag. For example: `niftty -e
...
///     the `niftty -e` command).
///
///   * `gtk-single-instance=false` - This ensures that a new instance is
///     launched and the CLI args are respected.
///
///   * `quit-after-last-window-closed=true` - This ensures that the Niftty
...
/// variables set by Niftty. For example, if you set `GHOSTTY_RESOURCES_DIR`
/// then the value you set here will override the value Niftty typically
/// automatically injects.
///
/// These environment variables _will not_ be passed to commands run by Niftty
...
/// memory. On supported systems with scrollback compression enabled, Niftty
...
/// pages. Compression does not allow Niftty to retain more history than the
...
///
/// This limit is an estimate. Internally, Niftty will only trim lines
...
///
/// Niftty does its best to only compress when idle and decompress
...
/// content-dependent.
///
/// Note that the way Niftty works is that we compress and discard the
...
/// The title Niftty will use for the window. This will force the title of the
/// window to be this title at all times and Niftty will ignore any set title
...
/// bus name that Niftty uses to connect to DBus.
///
/// Note that changing this value between invocations will create new, separate
/// instances, of Niftty when running with `gtk-single-instance=true`. See that
/// option for more details.
///
/// Changing this value may break launching Niftty from `.desktop` files, via
/// DBus activation, or systemd user services as the system is expecting Niftty
...
/// setting. If a previous Niftty terminal exists in the same process,
...
/// if Niftty can detect it is launched from launchd (double-clicked) or
/// `open`, then it defaults to `home`. On Linux with GTK, if Niftty can detect
...
/// the documentation or using the `niftty +list-actions` command.
///
...
/// otherwise bound. When looking up a binding, Niftty first tries to match
...
///
/// Warning: If you define a sequence as a CLI argument to `niftty`,
/// you probably have to quote the keybind since `>` is a special character
/// in most shells. Example: niftty --keybind='ctrl+a>n=new_window'
///
/// A trigger sequence has some special handling:
///
///   * Niftty will wait an indefinite amount of time for the next key in
...
///  * `global:`
///
///    Make the keybind global. By default, keybinds only work within Niftty
...
///    (e.g. even when Niftty is not focused), specify this prefix.
...
///    if there is a selection to copy. If there is no selection, Niftty
...
/// to Niftty. When a `global:` keybind is specified and Niftty is launched
/// or reloaded, Niftty will attempt to request these permissions.
...
/// Remap modifier keys within Niftty. This allows you to swap or reassign
...
///
/// This will NOT change keyboard layout or key encodings outside of Niftty.
...
/// before Niftty receives the key events.
///
/// Example:
///
...
///   prior to Niftty receiving them. For example, `option+a` on macOS
...
///   this happens before Niftty receives the key event. To workaround
...
/// may override this preference. Niftty will do its best to respect this
...
///    desktop environment. This option usually makes Niftty look the most
...
/// certain size to fit within the grid. There is nothing Niftty will do about
...
/// to be a certain position to fit within the grid. There is nothing Niftty
...
///
///   * `always` will always save window state whenever Niftty is exited.
///
/// If you change this value to `never` while Niftty is not running, the next
/// Niftty launch will NOT restore the window state.
///
/// If you change this value to `default` while Niftty is not running and the
/// previous exit saved state, the next Niftty launch will still restore the
/// window state. This is because Niftty cannot know if the previous exit was
/// due to a forced save or not (macOS doesn't provide this information).
///
/// If you change this value so that window state is saved while Niftty is not
/// running, the previous window state will not be restored because Niftty only
...
/// of an escape sequence hardcoded into Niftty for now.
///
...
/// this limit bounds how much memory a program can make Niftty
...
/// This is targeted directly at using Niftty from the CLI in a way
...
///
/// On Linux, if this is `true`, Niftty can delay quitting fully until a
/// configurable amount of time has passed after the last window is closed.
/// See the documentation of `quit-after-last-window-closed-delay`.
@"quit-after-last-window-closed": bool = builtin.os.tag == .linux,

/// Controls how long Niftty will stay running after the last open surface has
...
/// Niftty will quit immediately after the last window is closed if
/// `quit-after-last-window-closed` is `true`.
///
/// Only implemented on Linux.
@"quit-after-last-window-closed-delay": ?Duration = null,

/// This controls whether an initial window is created when Niftty
/// is run. Note that if `quit-after-last-window-closed` is `true` and
/// `quit-after-last-window-closed-delay` is set, setting `initial-window` to
/// `false` will mean that Niftty will quit after the configured delay if no
...
///
/// On macOS, changing this configuration requires restarting Niftty
/// completely.
///
...
/// Niftty uses the compositor-reported primary output when available and
...
///     Attempts to install Niftty's embedded terminfo entry using `tic` on local
...
/// during SSH sessions launched through Niftty's shell integration, including
...
/// Show a non-obstructive notification whenever Niftty automatically forwards
...
/// command-palette-entry = title:"Niftty",description:"Add a little Niftty to your terminal.",action:"text:\xf0\x9f\x91\xbb"
/// ```
///
/// There are some additional special values that can be specified for
...
/// Niftty currently supports OSC 10 (foreground), OSC 11 (background), and
...
///
/// Warning: Invalid shaders can cause Niftty to become unusable such as by
...
/// largely match Shadertoy, with some caveats and Niftty-specific extensions.
///
...
///
///    Request the user's attention when Niftty is unfocused, until it has
...
///      bring the Niftty window into focus;
///
...
/// Control the in-app notifications that Niftty shows.
///
...
/// and not something Niftty enables.
///
...
/// If true, Niftty on macOS will automatically enable the "Secure Input"
/// feature when it detects that a password prompt is being displayed.
///
/// "Secure Input" is a macOS security feature that prevents applications from
/// reading keyboard events. This can always be enabled manually using the
/// `Niftty > Secure Keyboard Entry` menu item.
///
...
/// If true, Niftty will show a graphical indication when secure input is
...
/// If true, Niftty exposes and handles the built-in AppleScript dictionary
/// on macOS.
///
...
/// Valid values:
///
///  * `official` - Use the official Niftty icon.
///  * `blueprint`, `chalkboard`, `microchip`, `glass`, `holographic`,
///    `paper`, `retro`, `xray` - Official variants of the Niftty icon
...
///  * `custom-style` - Use the official Niftty icon but with custom
...
/// Whether macOS Shortcuts are allowed to control Niftty.
///
/// Niftty exposes a number of actions that allow Shortcuts to
/// control and interact with Niftty. This includes creating new
...
/// Valid values are:
///
/// * `ask` - Ask the user whether for permission. Niftty will remember
///   this choice and never ask again. This is similar to other macOS
///   permissions such as microphone access, camera access, etc.
///
/// * `allow` - Allow Shortcuts to control Niftty without asking.
///
/// * `deny` - Deny Shortcuts from controlling Niftty.
///
/// Available since: 1.2.0
...
/// instead of the entire Niftty process. Similarly, if a shell program is
/// using too much CPU, only that surface will be CPU-throttled.
///
/// This will cause startup times to be slower (a hundred milliseconds or so),
/// so the default value is "single-instance." In single-instance mode, only
/// one instance of Niftty is running (see gtk-single-instance) so the startup
/// time is a one-time cost. Additionally, single instance Niftty is much
/// more likely to have many windows, tabs, etc. so cgroup isolation is a
/// big benefit.
///
/// This feature requires `systemd`. If `systemd` is unavailable, cgroup
/// initialization will fail. By default, this will not prevent Niftty from
...
///   * `single-instance` - Enable cgroups only for Niftty instances launched
...
/// If `true`, the Niftty GTK application will run in single-instance mode:
/// each new `niftty` process launched will result in a new window if there is
/// already a running process.
///
/// If `false`, each new niftty process will launch a separate application.
///
/// If `detect`, Niftty will assume true (single instance) unless one of
/// the following scenarios is found:
///
/// 1. TERM_PROGRAM environment variable is a non-empty value. In this
/// case, we assume Niftty is being launched from a graphical terminal
/// session and you want a dedicated instance.
///
/// 2. Any CLI arguments exist. In this case, we assume you are passing
/// custom Niftty configuration. Single instance mode inherits the
...
/// The default value is `detect`.
///
/// Note that debug builds of Niftty have a separate single-instance ID
...
/// If `true` (default), then the Niftty GTK tabs will be "wide." Wide tabs
...
///
/// Launch Niftty with `env GTK_DEBUG=interactive niftty` to tweak Niftty's
/// CSS in real time using the GTK Inspector. Errors in your CSS files would
/// also be reported in the terminal you started Niftty from. See
...
///
/// If the selected backend is not available on the platform, Niftty will
...
/// Control the auto-update functionality of Niftty. This is only supported
/// on macOS currently, since Linux builds are distributed via package
/// managers that are not centrally controlled by Niftty.
///
...
/// underlying protocols. To put it another way: Niftty doesn't explicitly
...
/// running Niftty version. If you download a pre-release version of Niftty
/// then this will be set to `tip` and you will receive pre-release updates.
/// If you download a stable version of Niftty then this will be set to
...
/// Set to true if Niftty was executed as xdg-terminal-exec on Linux.
...
pub fn finalize(self: *Config) !void {
    ...
    if (self.theme) |*theme| {
        ...
        if (different) {
            ...
            // because it'll force the theme based on the Niftty theme.
    ...
    // short value because it can cause Niftty to quit before the first
...
/// Niftty in a CLI environment. We need this to change some behaviors.
...
/// See macos-icon
///
/// Note: future versions of Niftty can support a custom icon with
