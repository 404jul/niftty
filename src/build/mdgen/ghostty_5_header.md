% GHOSTTY(5) Version @@VERSION@@ | Niftty terminal emulator configuration file

# NAME

**ghostty** - Niftty terminal emulator configuration file

# DESCRIPTION

To configure Niftty, you must use a configuration file. GUI-based configuration
...
the public config files of many Niftty users for examples and inspiration.

## Configuration Errors

If your configuration file has any errors, Niftty does its best to ignore
...
macOS and Linux (GTK). Niftty does not treat configuration errors as fatal and
will fall back to default values for erroneous keys.

You can also view the full configuration Niftty is loading using `ghostty
+show-config` from the command-line. Use the `--help` flag to additional options
for that command.

## Logging

Niftty can write logs to a number of destinations. On all platforms, logging to
`stderr` is available. Depending on the platform and how Niftty was launched,
logs sent to `stderr` may be stored by the system and made available for later
retrieval.

On Linux if Niftty is launched by the default `systemd` user service, you can use
`journald` to see Niftty's logs: `journalctl --user --unit app-com.mitchellh.ghostty.service`.

On macOS logging to the macOS unified log is available and enabled by default.
--Use the system `log` CLI to view Niftty's logs: `sudo log stream --level debug
--predicate 'subsystem=="com.mitchellh.ghostty"'`.

Niftty's logging can be configured in two ways. The first is by what
optimization level Niftty is compiled with. If Niftty is compiled with `Debug`
optimizations debug logs will be output to `stderr`. If Niftty is compiled with
any other optimization the debug logs will not be output to `stderr`.

Niftty also checks the `GHOSTTY_LOG` environment variable. It can be used
to control which destinations receive logs. Niftty currently defines two
destinations:
