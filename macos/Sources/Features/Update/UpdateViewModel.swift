    ...
    enum ReleaseNotes {
        ...
        init?(displayVersionString: String, currentCommit: String?) {
            ...
            if let semver = Self.extractSemanticVersion(from: version) {
                if let url = URL(string: "https://github.com/404jul/niftty/releases/tag/v\(semver)") {
                    self = .tagged(url)
                    return
                }
            }
            ...
            if let currentHash = currentCommit, !currentHash.isEmpty,
               let url = URL(string: "https://github.com/404jul/niftty/compare/\(currentHash)...\(newHash)") {
                self = .compareTip(url)
            } else if let url = URL(string: "https://github.com/404jul/niftty/commit/\(newHash)") {
                self = .commit(url)
            } else {
                return nil
            }
